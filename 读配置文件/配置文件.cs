...
public class 配置文件
{
    [JsonProperty(PropertyName = "启用插件", Order = -100)]
    internal bool 启用插件 = true;
    [JsonProperty(PropertyName = "是否隐藏没用到配置项的指令/ggw", Order = -99)]
    public bool 是否隐藏没用到的配置项 { get; set; } = false;
    [JsonProperty(PropertyName = "自定义强制隐藏哪些配置项", Order = -98)]
    public List<string> 自定义强制隐藏哪些配置项 = new List<string>();
    [JsonProperty(PropertyName = "控制台广告", Order = -97)]
    public bool 控制台广告 = false;

    public string 配置作者 = "GK 阁下";
    public string 配置说明 = "版本①;难度直线提升,怪物最低3个人血量,几乎全Boss加强!";
    public bool 启动错误报告 = false;
    public string 配置文件插件版本号 = "1.0.4.35";
    ...
    public bool 统一初始玩家系数额外增加 = false;
    public int 统一初始怪物玩家单体系数 = 0;
    ...
    {
        private readonly global::TestPlugin.配置文件 config;
